...
enum class ZK_NodeEvent {
  CreateNode,         //节点创建事件
  DeleteNode,         //节点删除事件
  ChangeValue,        //节点内容变更
  ChildNodeChange,    //子节点变化，新增、删除
  Session,            //会话事件
  DeleteWatch,           //监控删除事件
  Unknown
};
...
class ZookeeperClient {
 public:
  using Watch_cb=std::function<void(std::string_view path,ZK_NodeEvent event)>;

  explicit ZookeeperClient(std::string_view host) noexcept;
  ~ZookeeperClient() noexcept = default;

  void connect(time_t timeout,
               const std::function<void(ZK_SessionEvent)> &callback);
  ...
  /// @param data
  /// 存储到zk_path的数据，可用于识辨当前的master，因此可以考虑使用IP或其它有标识的值
  /// @return 如果竞争成功返回true，否则返回false
  bool raceMaster(std::string_view zk_path, std::string_view data) const;

  void raceSeqMaster(std::string_view zk_path, std::string_view data) const;
  ...
  /// @param nodeType 节点类型
  /// @param mode 节点权限
  void createNode(std::string_view path, std::string_view nodeName,
                  std::string_view data, ZK_NodeMode mode, ZK_ACL acl);
  ...
  /// @brief 设置节点数据，并校验版本号，若不一致则失败
  /// @param path 节点路径
  /// @param data 数据
  /// @param version 节点版本，-1则不校验
  void setData(std::string_view path,std::string_view data,int version=-1);

  void NodeWatch(std::string_view path,Watch_cb cb);

  void ChildNodeWatch(std::string_view path,Watch_cb cb);

  void delWatch(std::string_view path);
