...
static inline std::string getErrorInfo(int errcode) {
  return " zookeeper error," + std::string(zerror(errcode)) + " (" +
         std::to_string(errcode) + ")";
}

static ZK_SessionEvent ZKStatToSessionEvent(const int type) {
...
static ZK_NodeEvent ZKEventToNodeEvent(const int type) {
  if (ZOO_CREATED_EVENT == type) {
    return ZK_NodeEvent::CreateNode;
  } else if (ZOO_DELETED_EVENT == type) {
    return ZK_NodeEvent::DeleteNode;
  } else if (ZOO_CHANGED_EVENT == type) {
    return ZK_NodeEvent::ChangeValue;
  } else if (ZOO_CHILD_EVENT == type) {
    return ZK_NodeEvent::ChildNodeChange;
  } else if (ZOO_SESSION_EVENT == type) {
    return ZK_NodeEvent::Session;
  } else if (ZOO_NOTWATCHING_EVENT == type) {
    return ZK_NodeEvent::DeleteWatch;
  } else {
    return ZK_NodeEvent::Unknown;
  }
}
...
ZookeeperClient::ZookeeperClient(std::string_view host) noexcept
    : _zkHost(host),
      _zkClient(nullptr),
      _handle(nullptr, [this](zhandle_t* handle) { this->close(); }),
      _isConnected(false),
      _sessionCallback([](ZK_SessionEvent event) {}) {}

void ZookeeperClient::connect(
    time_t timeout, const std::function<void(ZK_SessionEvent)>& callback) {
  _handle.reset(zookeeper_init(_zkHost.c_str(), ZookeeperClient::zk_watcher,
                               timeout * 1000l, _zkClient.get(), this, 0));
...
void ZookeeperClient::close() {
  if (_handle) {
    ...
    if (errcode != ZOK) {
      throw zkError("zookeeper close node://" + _zkHost + "failed:" +
                    zerror(errcode) + "(" + std::to_string(errcode) + ")");
    }
    _handle.reset();
  }
}
...
std::string ZookeeperClient::getConnectedHost() const noexcept {
  ...
  if (nullptr == zookeeper_get_connected_host(
                     _handle.get(),
                     reinterpret_cast<struct sockaddr*>(&addr_in6),
                     &addr_len)) {
    return {};
  }
...
bool ZookeeperClient::raceMaster(std::string_view zk_path,
                                 std::string_view data) const {
  const int errcode = zoo_create(
      _handle.get(), zk_path.data(), data.data(), static_cast<int>(data.size()),
  ...
  if (ZNODEEXISTS == errcode) {
    ...
    throw zkError("raceMaster error," + std::string(zerror(errcode)) + "(" +
                  std::to_string(errcode) + ")");
  }
}
void ZookeeperClient::raceSeqMaster(std::string_view zk_path,
                                    std::string_view data) const {
  const int errcode = zoo_create(
    _handle.get(), zk_path.data(), data.data(), static_cast<int>(data.size()),
    &ZOO_OPEN_ACL_UNSAFE, ZOO_EPHEMERAL_SEQUENTIAL, nullptr, 0);
  if (ZOK == errcode) {
    return ;
  }
  if (ZNODEEXISTS == errcode) {
    ...
    throw zkError("raceSeqMaster error," + std::string(zerror(errcode)) + "(" +
                  std::to_string(errcode) + ")");
  }
}
void ZookeeperClient::createNode(std::string_view path, std::string_view node,
                                 std::string_view data, ZK_NodeMode mode,
                                 ZK_ACL acl) {
  ...
  const int errcode = zoo_create(_handle.get(), nodePath.c_str(), data.data(),
                                 static_cast<int>(data.size()), &aclVector,
                                 static_cast<int>(mode), nullptr, 0);
  if (errcode != ZOK) {
...
void ZookeeperClient::deleteNode(std::string_view nodePath, int version) {
  ...
  if (ZOK != errcode) {
    throw zkError("delete path://" + std::string(nodePath) +
                  detail::getErrorInfo(errcode));
  }
}
std::vector<std::string> ZookeeperClient::getAllChildren(
    std::string_view path) const {
  struct String_vector children{};
  std::vector<std::string> childrenVec;
  const int errcode =
      zoo_get_children(_handle.get(), path.data(), 0, &children);
  if (ZOK != errcode) {
    throw zkError("get path://" + std::string(path) + "children failed," +
                  detail::getErrorInfo(errcode));
  }
...
std::string ZookeeperClient::getData(std::string_view path, int maxSize,
                                     bool keep_watch) const {
  std::string data;
  data.resize(maxSize);
  struct Stat stat{};
  const int errcode = zoo_get(_handle.get(), path.data(), keep_watch ? 1 : 0,
                              data.data(), &maxSize, &stat);
  if (ZOK != errcode) {
    throw zkError("get path://" + std::string(path) + "data failed," +
                  detail::getErrorInfo(errcode));
  }
  if (maxSize > 0) {
    data.resize(maxSize);
...
void ZookeeperClient::setData(std::string_view path, std::string_view data,
                              int version) {
  const auto code =
      zoo_set(_handle.get(), path.data(), data.data(), data.size(), version);
  if (code != ZOK) {
    throw zkError("setData://" + std::string(path) + " failed," +
                  detail::getErrorInfo(code));
  }
}
void ZookeeperClient::NodeWatch(std::string_view path, Watch_cb cb) {
  const int errcode = zoo_wexists(
      _handle.get(), path.data(),
      [](zhandle_t* zh, int type, int state, const char* path,
         void* watcherCtx) {
        if (watcherCtx) {
          auto cb = static_cast<Watch_cb*>(watcherCtx);
          std::invoke(*cb, path, detail::ZKEventToNodeEvent(type));
        }
      },
      &cb, nullptr);
  if (ZOK != errcode) {
    throw zkError("NodeWatch://" + std::string(path) + " failed," +
                  detail::getErrorInfo(errcode));
  }
}
void ZookeeperClient::delWatch(std::string_view path) {
  const int errcode=zoo_remove_all_watches(_handle.get(),path.data(),ZWATCHTYPE_ANY,1);
  if (ZOK != errcode) {
    throw zkError("delWatch://" + std::string(path) + " failed," +
                  detail::getErrorInfo(errcode));
  }
}

void ZookeeperClient::zk_watcher(zhandle_t* zh, int type, int state,
...
void ZookeeperClient::waitConnected() noexcept {
  ...
  if (!_isConnected) {
    _cond.wait(lock, [this] { return _isConnected; });
  }
}
