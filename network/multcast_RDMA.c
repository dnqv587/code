...
/* Default parameter values */#define DEFAULT_PORT "51216"
...
int ret;struct context *ctx = (struct context *) arg;
while (1)
{
...
*Set this to NULL and the event will be acked automatically Otherwise the
caller must ack the event *using rdma_ack_cm_event
*
*Returns:
...
*/
int
resolve_addr (struct context *ctx)
{
int ret;
...
/* A PD is created when we bind. Copy it to the context so it can * be
used later on */
ctx->pd = ctx->id->pd;
}
ret =
rdma_resolve_addr (ctx->id, (bind_rai) ? bind_rai->ai_src_addr : NULL,
...
memcpy (&ctx->mcast_sockaddr, mcast_rai->ai_dst_addr,
...
*/
int
create_resources (struct context *ctx)
{
int ret, buf_size;
struct ibv_qp_init_attr attr;
memset (&attr, 0, sizeof (attr));
/* If we are bound to an address, then a PD was already allocated * to the
CM ID */
if (!ctx->pd)
{
...
return ret;}
/* The receiver must allow enough space in the receive buffer for * the
GRH */
...
* 0 on success, non-zero on failure
*Description:
*Destroyp AH,QP,CQ,MR,PD and ID
*/
void
...
* 0 on success, non-zero on failure
* Description:
* Posts a UD send to the multicast add ress
* * * * */
int
post_send (struct context *ctx)
{
int ret;
...
/* Multicast requires that the message is sent with immediate data * and
that the QP number is the contents of the immediate data */
wr.next = NULL;
wr.sg_list = &sge;
wr.num_sge = 1;
...
* * * */
int
get_completion (struct context *ctx)
{
int ret;
struct ibv_wc wc;
do
...
/*
*接收端
*./multcast_RDMA -m 239.100.100.100 -b 100.100.1.241
*发送端
*./multcast_RDMA -m 239.100.100.100 -s 1 -b 100.100.1.242
* Function: main
* Input:
...
while ((op = getopt (argc, argv, "s:h:b:m:p:c:l:")) != -1)
{
switch (op)
{
case 's':
...
break;
case 'c':
ctx.msg_count = atoi (optarg);break;
case 'l':
...
{
ret =
rdma_post_recv (ctx.id, NULL, ctx.buf,
...
if (!ctx.ah)
{
VERB_ERR ("ibv_create_ah", -1);goto out;
}
}
rdma_ack_cm_event (event);
